...
async function runCuriousPuppeteerScript(targetOrg) {
  ...
  try {
    ...
    for (const slug of slugList) {
      ...
      // Insert citizen data into SQLite using Sequelize
      ...
      await delay(1000); // Add a delay to avoid rate limiting
    }
    ...
    return csvData; // Return the CSV data instead of the file path
  } catch (error) {
    console.error('Error in runCuriousPuppeteerScript:', error);
    await sequelize.close();
