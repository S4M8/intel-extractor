...
const main = async () => {
    ...
    // const accountButton = await page.waitForSelector('#platform-bar > section > div > nav > ul.c-platform-list.c-platform-list--rsi.c-platform-navigation-list > li:nth-child(3) > a');
    // await accountButton.click()

    await page.locator('#platform-bar > section > div > nav > ul.c-platform-list.c-platform-list--rsi.c-platform-navigation-list > li:nth-child(3) > a').click()
    ...
    await page.locator('#enlist-root > div > div > div.c-formLegacyEnlist__wrapper > form > div > button')
              .click()
    ...
    // await page.goto("https://robertsspaceindustries.com/orgs/SPAM/members")
    await page.goto("https://robertsspaceindustries.com/orgs/JARALEWES/members")

    await delay(2000)
    
    const membersContainer = await page.$('#members-data');
    const totalMembersElement = await page.$('.totalrows.js-totalrows');
    const totalMembers = parseInt(await totalMembersElement.evaluate(el => el.textContent.trim()), 10);
    const elementSelector = '.c-platform-copyright.c-platform-copyright--rsi';
    const cardData = [];
    
    async function scrollElementIntoView(page: { $: (arg0: any) => any; viewport: () => { (): any; new(): any; height: number; }; }, elementSelector: string) {
        try {
        ...
        for (const cardElement of newCards) {
          try {
            ...
            if (!scrapedMembers.has(href) && href) { // Check if already scraped
              ...
              const nameAndNickname = await cardElement.$eval('.name-wrap', el => ({
      ...
      for (let i = 0; i < pageLoads; i++) {
        ...
        await delay(1000);
        const loader = await page.$('.traj-loader.trans-02s[style="opacity: 0;"]') !== null;
        if (loader) {
    ...
    async function scrapeCitizenInfo(href, cardData) {
      try {
          ...
          const mainOrg = await page.$eval('.info > p > a', el => el.textContent.trim());
          const altOrg = `${href}/organizations`;
          
          const regionParts = await page.$$eval('.left-col > div > :nth-child(2) > strong', els =>
          els.map(el => el.textContent.trim().replace(/\s+/g, '')));
          const region = regionParts.join(', '); 
          
          const fluency = await page.$eval('.left-col > div > :nth-child(3) > strong', el => el.textContent.trim());
